...
import { useProductSelect2, useCategorySelect2, useEmployeeSelect2 } from '@/modules/inventory/hooks/useSelect2'
...
// --- Main PurchaseRequisitionCreatePage Component ---
export const PurchaseRequisitionCreatePage = () => {
  const navigate = useNavigate()
  const { user } = useAuthStore()
  const { currency, currencyPosition } = useSettings()
  const { mutate: createPR, isPending } = useCreatePurchaseRequisition()
  ...
  const defaultRequiredBy = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0]
  ...
  const {
    ...
    defaultValues: {
      pr_date: todayStr,
      required_by_date: defaultRequiredBy,
      priority: 'medium',
      procurement_type: 'normal',
      is_emergency: false,
      requisitioner_name: user ? `${user.first_name || ''} ${user.last_name || ''}`.trim() : '',
      email: user?.email || '',
      items: [
        {
          ...
          item_description: '',
          budget_head_id: undefined as any,
          quantity: 1,
          estimated_rate: 0,
        },
      ],
  ...
  const onSubmit = (data: PurchaseRequisitionFormValues) => {
    const payload = {
      ...data,
      is_emergency: data.procurement_type === 'emergency',
      attachments: selectedFiles,
    }

    createPR(payload as any, {
      onSuccess: (res) => {
        showNotificationModal(
          'Purchase Requisition Created!',
          res.message || 'Draft purchase requisition created successfully.',
          'success'
        )
      ...
      onError: (error: any) => {
        ...
        if (backendErrors && typeof backendErrors === 'object') {
          // Set errors on specific form fields so they highlight in red
          ...
          const message = error.response?.data?.message || error.message || 'Failed to create purchase requisition.'
          ...
          <div className="lg:col-span-9 flex flex-col gap-6">
            ...
            <div className="bg-white rounded-xl border border-primary/10 p-5 shadow-sm">
              ...
              <div className="grid grid-cols-1 md:grid-cols-12 gap-5">
                ...
                {/* Attachments Upload (Directly in Header) */}
                <div className="md:col-span-6 space-y-1.5">
                  <label className="text-[13px] font-semibold text-[#475569]">
