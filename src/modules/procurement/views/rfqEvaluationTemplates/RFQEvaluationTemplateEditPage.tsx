...
export const RFQEvaluationTemplateEditPage = () => {
  ...
  const { data: templateResponse, isLoading: isFetching, isError } = useRFQEvaluationTemplateDetails(id)
  const template = templateResponse?.data || (templateResponse as any)?.response || templateResponse

  const { mutate: updateTemplate, isPending } = useUpdateRFQEvaluationTemplate()
  ...
  const {
    ...
    defaultValues: {
      ...
      criteria: [
        { name: '', category: 'technical', max_score: 50 },
      ],
    },
  })
  ...
  // Populate data when template is fetched
  useEffect(() => {
    if (template) {
      const criteria = Array.isArray(template.criteria) && template.criteria.length > 0
        ? template.criteria.map((c: any) => ({
            name: c.name || '',
            category: (c.category as any) || 'technical',
            max_score: Number(c.max_score) || 50,
          }))
        : [{ name: '', category: 'technical' as const, max_score: 50 }]

      reset({
        name: template.name || '',
        technical_weightage: Number(template.technical_weightage) || 50,
        commercial_weightage: Number(template.commercial_weightage) || 50,
        criteria,
      })
    }
  }, [template, reset])
  ...
  if (isFetching) {
    return (
      <div className="min-h-[400px] flex items-center justify-center">
        <LoadingState message="Loading evaluation template details..." />
      </div>
    )
  }
    ...
    return (
      <div className="min-h-[400px] flex flex-col items-center justify-center gap-4 text-center p-6">
        ...
        <div className="space-y-1">
          <h2 className="text-lg font-bold text-gray-900">Template Not Found</h2>
          <p className="text-sm text-gray-500">The requested RFQ evaluation template could not be loaded.</p>
        </div>
        <button
          onClick={() => navigate({ to: '/procurement/rfq-templates' })}
