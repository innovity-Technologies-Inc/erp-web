...
import { useInvoiceReturnDetails } from '../../hooks/useReturn'
import { useSettings } from '@/hooks/useSettings'
import { formatCurrency, formatDate } from '@/utils/formatters'
import { LoadingState } from '@/components/Loading/LoadingState'

export const WastageDetailsPage = () => {
  const navigate = useNavigate()
  const { id } = useParams({ from: '/_authenticated/inventory/return/wastage/details/$id' })
  const { currency, currencyPosition, companyInformation, webSetting } = useSettings()

  const { data: response, isLoading } = useInvoiceReturnDetails(id || null)
  ...
  if (isLoading) {
    return <LoadingState message="Loading wastage details..." />
  }
    ...
    return (
      <div className="min-h-screen bg-[#f8fafc] flex items-center justify-center p-6 text-center">
        <div className="bg-white p-10 rounded-2xl border border-gray-200 shadow-sm max-w-md w-full">
           ...
           <h2 className="text-[20px] font-bold text-gray-900 mb-3 tracking-tight">Record Not Found</h2>
           <p className="text-[#64748b] text-[14px] leading-relaxed mb-8">The requested wastage record could not be found or has been removed from the system.</p>
           <button 
             onClick={() => navigate({ to: '/inventory/return/wastage' })}
  ...
  // Use the first item to get common data (invoice, customer, dates)
    ...
    <div className="min-h-screen font-poppins print:bg-white print:pb-0">
      ...
      <div className="flex items-center justify-between pb-6 print:hidden">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-[20px] font-medium text-primary tracking-tight ml-2">Wastage Details</h1>
        </div>
      ...
      <div className="max-w-[1200px] mx-auto bg-white rounded-xl border border-gray-200 shadow-sm print:shadow-none print:border-none print:m-0 print:max-w-none mb-8">
        ...
        <div className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 print:flex-row print:items-center print:p-4">
          ...
          <div className="text-right">
            <h2 className="text-[20px] font-semibold text-gray-900 tracking-tight mb-1 print:text-[18px] print:mb-0">Return Invoice #{invoice?.id || 'N/A'}</h2>
            <p className="text-[11px] font-bold text-gray-400 tracking-widest uppercase mb-2">Loss ID: {firstItem.return_id || 'N/A'}</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 print:grid-cols-2 gap-6 px-8 pb-6 print:px-4 print:pb-4 print:gap-4">
          {/* Billing From (Merchant) */}
          <div className="relative p-4 bg-white rounded-xl border border-gray-200 print:break-inside-avoid print:p-3">
              <div className="flex justify-between items-start mb-4 print:mb-2">
                <span className="inline-block px-3 py-1 bg-blue-100 text-[#1e4ba1] text-[10px] font-bold uppercase tracking-wider rounded-full">Reporting From</span>
              ...
              <h3 className="text-[18px] font-semibold text-gray-900 mb-4 print:text-[16px] print:mb-2">{customer?.customer_name || 'Merchant Name'}</h3>
              ...
              <div className="space-y-3 print:space-y-1.5">
                <div className="flex items-start gap-3 text-[13px] text-gray-500 font-medium leading-relaxed print:text-[11px]">
                  ...
                  <span>{customer?.customer_address || 'Merchant address not available'}</span>
                </div>
                <div className="flex items-center gap-3 text-[13px] text-gray-500 font-medium print:text-[11px]">
                  ...
                  <span>{customer?.customer_mobile || 'No phone provided'}</span>
                </div>
              </div>
          </div>
          ...
          <div className="relative p-4 bg-white rounded-xl border border-gray-200 print:break-inside-avoid print:p-3">
              <div className="flex justify-between items-start mb-4 print:mb-2">
                <span className="inline-block px-3 py-1 bg-orange-100 text-orange-600 text-[10px] font-bold uppercase tracking-wider rounded-full">Wastage Reported By</span>
              ...
              <h3 className="text-[18px] font-semibold text-gray-900 mb-4 print:text-[16px] print:mb-2">{companyInformation?.company_name || 'GEN-ITECH ERP'}</h3>
        ...
        <div className="border-t border-gray-100 mx-8 py-4 print:mx-4 print:py-2">
            <div className="flex items-center justify-between mb-4 print:mb-2">
              <h3 className="text-[16px] font-semibold text-gray-900 print:text-[14px]">Discarded Items</h3>
              <span className="text-[11px] font-bold text-gray-500 uppercase tracking-widest">{returnItems.length} {returnItems.length === 1 ? 'Item' : 'Items'} Found</span>
            </div>
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="bg-[#dae8ff] text-[#003671] text-[12px] font-bold uppercase tracking-wider border-b border-gray-100 print:bg-gray-50 text-center">
                      ...
                      <th className="px-4 py-3 border-b border-gray-200 w-[150px] text-right">Total Loss</th>
                    </tr>
                  </thead>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 print:grid-cols-12 gap-8 p-8 pt-2 print:gap-4 print:px-4 print:py-2 print:mt-2 items-start">
          ...
          <div className="lg:col-span-8 print:col-span-7 flex flex-col gap-6">
            <div className="bg-white rounded-xl border border-primary/10 p-5 shadow-sm flex flex-col gap-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[14px] font-bold text-gray-900">Wastage / Loss</span>
                </div>
            </div>
          ...
          <div className="lg:col-span-4 print:col-span-5 flex flex-col print:break-inside-avoid">
              <div className="bg-[#f8fafc] border border-gray-200 rounded-t-xl flex-grow print:bg-[#f8fafc]">
                <h3 className="text-[16px] font-semibold text-[#1e4ba1] p-4 print:text-[14px] print:p-2">Wastage Summary</h3>
              ...
              <div className="bg-[#0f2d5c] p-5 flex items-center justify-between text-white rounded-b-xl border border-[#0f2d5c] print:bg-[#0f2d5c] print:p-3">
                <span className="text-[13px] font-bold tracking-wider print:text-[11px]">TOTAL LOSS</span>
