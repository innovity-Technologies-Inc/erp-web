...
import { useInvoicePaymentDetails } from '../../hooks/useSales'
import { useSettings } from '@/hooks/useSettings'
import { formatCurrency, formatDate } from '@/utils/formatters'
import { LoadingState } from '@/components/Loading/LoadingState'

export const InvoicePaymentViewPage = () => {
  const navigate = useNavigate()
  const { id } = useParams({ from: '/_authenticated/inventory/sales/payments/$id' })
  const { currency, currencyPosition, companyInformation, webSetting } = useSettings()

  const paymentId = id ? parseInt(id as string, 10) : null
  const { data: payment, isLoading } = useInvoicePaymentDetails(paymentId)
    ...
    return (
      <div className="min-h-screen bg-[#f8fafc] flex items-center justify-center p-6 text-center font-poppins">
        <div className="bg-white p-10 rounded-2xl border border-gray-200 shadow-sm max-w-md w-full">
           ...
           <h2 className="text-[20px] font-bold text-gray-900 mb-3 tracking-tight">Payment Record Not Found</h2>
           <p className="text-[#64748b] text-[14px] leading-relaxed mb-8">The requested payment record could not be found or has been removed from the system.</p>
           <button 
             onClick={() => navigate({ to: '/inventory/sales/payments' })}
  ...
  // Robustly extract items handling both camelCase and snake_case from backend
  const items = Array.isArray(payment.invoicePaymentDetails) 
    ? payment.invoicePaymentDetails 
    : Array.isArray(payment.invoice_payment_details) 
  ...
  return (
    <div className="min-h-screen font-poppins print:bg-white print:pb-0">
      {/* Page Header - Standardized to match SaleViewPage */}
      <div className="flex items-center justify-between pb-6 print:hidden">
        <div className="flex items-center gap-4">
          <Link 
            to="/inventory/sales/payments"
          ...
          <h1 className="text-[20px] font-medium text-primary tracking-tight ml-2">Payment Details</h1>
        </div>
      ...
      <div className="max-w-[1200px] mx-auto bg-white rounded-xl border border-gray-200 shadow-sm print:shadow-none print:border-none print:m-0 print:max-w-none mb-8">
        ...
        <div className="p-6 flex flex-col md:flex-row md:items-center justify-between gap-6 print:flex-row print:items-center print:p-4">
          ...
          <div className="text-right">
            <h2 className="text-[20px] font-semibold text-gray-900 tracking-tight mb-2 print:text-[18px] print:mb-1">Receipt #{payment.payment_ref}</h2>
            <div className="flex items-center justify-end gap-4 text-[#64748b] text-[11px] font-medium print:gap-2">
              ...
              <div className="flex items-center gap-1.5">
                  ...
                  {new Date(payment.created_at).toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false })}
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 print:grid-cols-2 gap-6 px-8 pb-6 print:px-4 print:pb-4 print:gap-4">
          ...
          <div className="relative p-4 bg-white rounded-xl border border-gray-200 print:break-inside-avoid print:p-3">
              ...
              <h3 className="text-[18px] font-semibold text-gray-900 mb-4 print:text-[16px] print:mb-2">{payment.customer?.customer_name || payment.customer_name || 'Merchant Name'}</h3>
              
              <div className="space-y-3 print:space-y-1.5">
                <div className="flex items-start gap-3 text-[13px] text-gray-500 font-medium leading-relaxed print:text-[11px]">
                  <MapPin className="w-4 h-4 mt-0.5 shrink-0 print:w-3.5 print:h-3.5" />
                  <span>{payment.customer?.customer_address || 'Customer address not available'}</span>
                </div>
                <div className="flex items-center gap-3 text-[13px] text-gray-500 font-medium print:text-[11px]">
                  <Phone className="w-4 h-4 shrink-0 print:w-3.5 print:h-3.5" />
                  <span>{payment.customer?.customer_mobile || 'No phone provided'}</span>
                </div>
                <div className="flex items-center gap-3 text-[13px] text-gray-500 font-medium print:text-[11px]">
                  <Mail className="w-4 h-4 shrink-0 print:w-3.5 print:h-3.5" />
                  <span>{payment.customer?.customer_email || 'No email provided'}</span>
                </div>
              </div>
          </div>
        </div>

        {/* Ordered Products Section - Matches SaleView table style */}
        <div className="border-t border-gray-100 mx-8 py-4 print:mx-4 print:py-2">
            <div className="flex items-center justify-between mb-4 print:mb-2">
              <h3 className="text-[16px] font-semibold text-gray-900 print:text-[14px]">Payment Item Specifications</h3>
              <span className="text-[11px] font-bold text-gray-500 uppercase tracking-widest">{items.length} {items.length === 1 ? 'Item' : 'Items'} Recorded</span>
            </div>
                      ...
                      <tr key={index} className="hover:bg-gray-50/50 transition-colors print:bg-white">
                        ...
                        <td className="px-4 py-4 print:px-2 print:py-2">
                          <div className="flex flex-col gap-0.5">
                            ...
                            <span className="text-[11px] text-gray-400 font-medium print:text-[9px]">SKU: {item.product?.sku || 'N/A'}</span>
                          </div>
                        </td>
                        ...
                        <td className="px-4 py-4 print:px-2 print:py-2">
                          <span className="px-2.5 py-1 bg-gray-100 text-gray-600 text-[11px] font-medium rounded-md print:bg-gray-100 print:text-[9px] print:px-1.5 print:py-0.5">{item.product?.unit?.unit_name || item.product?.unit || 'Unit'}</span>
                        </td>
                        <td className="px-4 py-4 text-center font-semibold text-gray-900 print:px-2 print:py-2">{parseFloat(item.product_quantity).toFixed(2)}</td>
                        <td className="px-4 py-4 text-right text-gray-700 print:px-2 print:py-2">{formatValue(item.product_rate)}</td>
                        <td className="px-4 py-4 text-center text-rose-500 font-semibold print:px-2 print:py-2">{formatValue(item.discount)}</td>
                        <td className="px-6 py-4 text-right font-bold text-[#1e4ba1] text-[14px] print:text-[12px] print:px-3 print:py-2">{formatValue(item.total_value)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 print:grid-cols-12 gap-8 p-8 pt-2 print:gap-4 print:px-4 print:py-2 print:mt-2">
          ...
          <div className="lg:col-span-8 print:col-span-7 space-y-6 print:space-y-3">
              {payment.details && (
                <div className="p-5 bg-[#f8fafc] rounded-xl border border-gray-200 print:bg-[#f8fafc] print:p-3">
                    <div 
                      ...
                      dangerouslySetInnerHTML={{ __html: payment.details }}
                    />
                </div>
              )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 print:grid-cols-2 gap-4 print:gap-2">
                ...
                <div className="p-4 bg-white rounded-xl border border-gray-200 flex items-center gap-4 print:break-inside-avoid print:p-2 print:gap-2">
                  ...
                  <div className="flex flex-col">
                    <span className="text-[11px] font-medium text-gray-400 mb-0.5 print:text-[9px]">Invoice Reference</span>
                    <span className="text-[14px] font-bold text-gray-900 print:text-[12px]">{payment.invoice_id}</span>
                  </div>
                </div>
              </div>
          </div>

          {/* Right: Payment Summary - Matches SaleView exactly */}
          <div className="lg:col-span-4 print:col-span-5 flex flex-col print:break-inside-avoid">
              <div className="bg-[#f8fafc] border border-gray-200 rounded-t-xl flex-grow print:bg-[#f8fafc]">
                ...
                <div className="space-y-3.5 bg-white text-[14px] p-4 print:text-[12px] print:p-2 print:space-y-1.5">
                  <div className="flex justify-between items-center">
                    ...
                    <span className="text-gray-900 font-semibold">{formatValue(payment.total_amount)}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-gray-500">Discount Value:</span>
                    <span className="text-rose-600 font-semibold">- {formatValue(payment.total_discount || 0)}</span>
                  </div>
                  <div className="flex justify-between items-center pt-3 pb-3 border-t border-b border-gray-200 my-2 print:pt-1 print:pb-1 print:my-1">
                    ...
                    <span className="text-[16px] text-[#1e4ba1] font-bold print:text-[14px]">{formatValue(payment.total_amount - (payment.total_discount || 0))}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-gray-500">Transaction Ref:</span>
                    <span className="text-gray-500 font-medium">{payment.transaction_ref}</span>
                  </div>
                  <div className="flex justify-between items-center">
                    <span className="text-gray-500">Paid Amount:</span>
                    <span className="text-[#1e4ba1] font-semibold">{formatValue(payment.paid_amount)}</span>
                  </div>
                  ...
                  <div className="mt-4 p-4 bg-rose-50 rounded-lg border border-rose-100 flex items-center justify-between print:bg-rose-50 print:p-2 print:mt-1">
                    ...
                    <span className="text-[16px] font-bold text-rose-600 print:text-[14px]">{formatValue(payment.due_amount)}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-[#0f2d5c] p-5 flex items-center justify-between text-white rounded-b-xl border border-[#0f2d5c] print:bg-[#0f2d5c] print:p-3">
                <span className="text-[13px] font-bold tracking-wider print:text-[11px]">NET RECOVERABLE</span>
                <span className="text-[20px] font-bold print:text-[16px]">{formatValue(payment.due_amount)}</span>
              </div>
          </div>
