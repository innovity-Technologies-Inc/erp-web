...
import { useWarehouseWiseStockReportDatatable } from '../../hooks/useReports'
...
import type { WarehouseWiseStockReportListItem } from '../../api/reports.api'
import type { ColDef } from 'ag-grid-community'

export const WarehouseWiseStockReportPage = () => {
  ...
  const productOptions = useMemo(() => {
    const rawData = productsResponse as any
    const data = Array.isArray(rawData) ? rawData : rawData?.data || []
    return data.map((p: any) => ({ value: p.id.toString(), label: p.product_name || p.text || p.name }))
  }, [productsResponse])

  const warehouseOptions = useMemo(() => {
    const rawData = warehousesResponse as any
    const data = Array.isArray(rawData) ? rawData : rawData?.data || []
    return data.map((w: any) => ({ value: w.id.toString(), label: w.name || w.text }))
  }, [warehousesResponse])
  ...
  }), [currentPage, pageSize, warehouseId, productId])

  const { data: reportData, isFetching: isLoading } = useWarehouseWiseStockReportDatatable(params)
  ...
  const totals = useMemo(() => {
    ...
    return pageData.reduce((acc, item) => ({
      ...
      total_sale_return_qnty: acc.total_sale_return_qnty + (parseFloat(String(item.total_sale_return_qnty).replace(/,/g, '')) || 0),
      stok_quantity: acc.stok_quantity + (parseFloat(String(item.stok_quantity).replace(/,/g, '')) || 0),
      ...
      purchase_total: acc.purchase_total + (parseFloat(String(item.purchase_total).replace(/,/g, '')) || 0),
        ...
        total_sale_return_qnty: 0, stok_quantity: 0, 
  ...
  const gridData = useMemo(() => {
    ...
    const summaryRow = {
      ...
      total_sale_return_qnty: totals.total_sale_return_qnty.toString(),
      stok_quantity: totals.stok_quantity.toString(),
  ...
  const columnDefs = useMemo<ColDef<WarehouseWiseStockReportListItem>[]>(() => [
    { 
      headerName: 'SL', 
    ...
    { 
      ...
      minWidth: 150,
      flex: 1,
      hide: !visibleCols.warehouse_name,
      cellClass: 'text-center flex items-center justify-center text-gray-500 font-bold uppercase tracking-tight'
    },
    { 
    ...
    { 
      ...
      field: 'total_sale_return_qnty',
      minWidth: 100,
      flex: 1,
      hide: !visibleCols.total_sale_return_qnty,
    ...
    { 
      ...
      minWidth: 100,
      flex: 1,
      hide: !visibleCols.stok_quantity,
  ...
  const filterColumns = [
    ...
    { name: 'Warehouse', field: 'warehouse_name', visible: visibleCols.warehouse_name },
    ...
    { name: 'Out Ret Qty', field: 'total_sale_return_qnty', visible: visibleCols.total_sale_return_qnty },
    { name: 'Stock', field: 'stok_quantity', visible: visibleCols.stok_quantity },
  ...
  const handlePdfExport = async () => {
    try {
      ...
      const response = await apiClient.get('/inventory/reports/warehouse-wise-stock-export', {
        params: { report_type: 'pdf', warehouse_id: warehouseId, product_id: productId },
        responseType: 'blob'
      })
  ...
  const handleExcelExport = async () => {
    try {
      ...
      const response = await apiClient.get('/inventory/reports/warehouse-wise-stock-export', {
        params: { report_type: 'excel', warehouse_id: warehouseId, product_id: productId },
        responseType: 'blob'
      })
      const url = window.URL.createObjectURL(new Blob([response.data]))
      const link = document.createElement('a')
      link.href = url
      link.setAttribute('download', `warehouse_stock_report_${new Date().toISOString().split('T')[0]}.xlsx`)
    ...
    <div className="flex items-center gap-3">
        <div className="w-[180px]">
            <Select2
                ...
                value={warehouseId}
                onChange={(val) => { setWarehouseId(val as string); setCurrentPage(1) }}
                rounded="full"
                variant="solid"
  ...
  return (
    <ListPageLayout<WarehouseWiseStockReportListItem>
