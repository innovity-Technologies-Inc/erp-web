import { useMemo, useState } from 'react'
import { FileDown, FileSpreadsheet, Loader2 } from 'lucide-react'
import { useStockMovementReportDatatable, useStockMovementFilterOptions } from '../../hooks/useReports'
import { ListPageLayout } from '@/components/ListPageLayout/ListPageLayout'
import { useUiStore } from '@/store/useUiStore'
import { stockReportOptions, reportCategoryTabs } from './constants'
import { apiClient } from '@/api/client'
import { Select2 } from '@/components/Select/Select2'
import type { StockMovementReportListItem } from '../../api/reports.api'
import type { ColDef } from 'ag-grid-community'

export const StockMovementReportPage = () => {
  const [currentPage, setCurrentPage] = useState(1)
  const [pageSize, setPageSize] = useState(10)
  const { showNotificationModal } = useUiStore()

  // Filters
  const [batchId, setBatchId] = useState<string>('')
  const [productId, setProductId] = useState<string>('')
  const [movementType, setMovementType] = useState<string>('')

  const { data: filterOptions } = useStockMovementFilterOptions()
  
  const productOptions = useMemo(() => {
    return (filterOptions?.products || []).map((p: any) => ({ value: p.id.toString(), label: p.product_name }))
  }, [filterOptions])

  const batchOptions = useMemo(() => {
    return (filterOptions?.batches || []).map((b: any) => ({ value: b.id.toString(), label: b.batch_no }))
  }, [filterOptions])

  const movementTypeOptions = useMemo(() => {
    return (filterOptions?.movement_types || []).map((t: any) => ({ 
      value: t.id.toString(), 
      label: `${t.type_name} [${t.category}]` 
    }))
  }, [filterOptions])

  const [visibleCols, setVisibleCols] = useState({
    sl: true,
    batch_no: true,
    product_name: true,
    movement_type: true,
    quantity: true,
    source_warehouse: true,
    destination_warehouse: true,
    reference: true,
  })

  const toggleColumn = (field: string) => {
    setVisibleCols((prev: any) => ({ ...prev, [field]: !prev[field] }))
  }

  const [isExportingPdf, setIsExportingPdf] = useState(false)
  const [isExportingExcel, setIsExportingExcel] = useState(false)

  const params = useMemo(() => ({
    start: (currentPage - 1) * pageSize,
    length: pageSize,
    batch_master_id: batchId,
    product_id: productId,
    movement_type: movementType,
  }), [currentPage, pageSize, batchId, productId, movementType])

  const { data: reportData, isFetching: isLoading } = useStockMovementReportDatatable(params)

  // Calculate totals from the current page data
  const totals = useMemo(() => {
    const pageData = reportData?.data || []
    return pageData.reduce((acc, item) => ({
      quantity: acc.quantity + (parseFloat(String(item.quantity).replace(/,/g, '')) || 0),
    }), { quantity: 0 })
  }, [reportData?.data])

  // Prepare data with in-grid summary row appended
  const gridData = useMemo(() => {
    const data = (reportData?.data || []) as any[]
    if (data.length === 0) return data

    const summaryRow = {
      isSummary: true,
      movement_type: 'Total:',
      quantity: totals.quantity.toString(),
    }

    return [...data, summaryRow]
  }, [reportData?.data, totals])

  const columnDefs = useMemo<ColDef<StockMovementReportListItem>[]>(() => [
    { 
      headerName: 'SL', 
      valueGetter: (params: any) => {
        if ((params.data as any)?.isSummary) return ''
        return (currentPage - 1) * pageSize + (params.node?.rowIndex ?? 0) + 1
      },
      width: 60,
      flex: 0,
      pinned: 'left' as const,
      hide: !visibleCols.sl,
      cellClass: 'text-gray-400 font-medium border-r border-primary/30 flex items-center justify-center',
    },
    { 
      headerName: 'BATCH NO', 
      field: 'batch_no',
      minWidth: 120,
      flex: 1,
      hide: !visibleCols.batch_no,
      cellClass: 'text-center flex items-center justify-center text-gray-500 font-bold'
    },
    { 
      headerName: 'PRODUCT NAME', 
      field: 'product_name',
      minWidth: 200,
      flex: 2,
      hide: !visibleCols.product_name,
      cellClass: 'text-left flex items-center px-4 font-medium text-[#475569]'
    },
    { 
      headerName: 'MOVEMENT TYPE', 
      field: 'movement_type',
      minWidth: 150,
      flex: 1.5,
      hide: !visibleCols.movement_type,
      cellClass: (params) => {
        const isSum = (params.data as any)?.isSummary
        return `font-medium flex items-center ${isSum ? 'justify-end pr-4 text-[#1e293b] font-bold' : 'text-[#475569] px-4'}`
      },
    },
    { 
      headerName: 'QUANTITY', 
      field: 'quantity',
      minWidth: 120,
      flex: 1,
      hide: !visibleCols.quantity,
      cellClass: (params) => `font-black flex items-center justify-center ${(params.data as any)?.isSummary ? 'text-[#1e293b]' : 'text-[#1e4ba1]'}`,
      valueFormatter: (params: any) => params.value ? parseFloat(params.value).toLocaleString() : '0'
    },
    { 
      headerName: 'SOURCE', 
      field: 'source_warehouse',
      minWidth: 150,
      flex: 1.5,
      hide: !visibleCols.source_warehouse,
      cellClass: 'text-center flex items-center justify-center text-gray-500 font-medium'
    },
    { 
      headerName: 'DESTINATION', 
      field: 'destination_warehouse',
      minWidth: 150,
      flex: 1.5,
      hide: !visibleCols.destination_warehouse,
      cellClass: 'text-center flex items-center justify-center text-gray-500 font-medium'
    },
    { 
      headerName: 'REFERENCE', 
      field: 'reference',
      minWidth: 150,
      flex: 1.5,
      hide: !visibleCols.reference,
      cellClass: 'text-center flex items-center justify-center text-[#1e4ba1] font-bold'
    }
  ], [visibleCols, currentPage, pageSize])

  const filterColumns = [
    { name: 'SL', field: 'sl', visible: visibleCols.sl },
    { name: 'Batch No', field: 'batch_no', visible: visibleCols.batch_no },
    { name: 'Product Name', field: 'product_name', visible: visibleCols.product_name },
    { name: 'Movement Type', field: 'movement_type', visible: visibleCols.movement_type },
    { name: 'Quantity', field: 'quantity', visible: visibleCols.quantity },
    { name: 'Source', field: 'source_warehouse', visible: visibleCols.source_warehouse },
    { name: 'Destination', field: 'destination_warehouse', visible: visibleCols.destination_warehouse },
    { name: 'Reference', field: 'reference', visible: visibleCols.reference },
  ]

  const handlePdfExport = async () => {
    try {
      setIsExportingPdf(true)
      const response = await apiClient.get('/inventory/reports/stock-movement-export', {
        params: { report_type: 'pdf', batch_master_id: batchId, product_id: productId, movement_type: movementType },
        responseType: 'blob'
      })
      const url = window.URL.createObjectURL(new Blob([response.data], { type: 'application/pdf' }))
      window.open(url, '_blank')
      setTimeout(() => window.URL.revokeObjectURL(url), 1000)
    } catch (error) {
      console.error('Failed to export PDF:', error)
      showNotificationModal('Error!', 'Failed to export PDF report.', 'error')
    } finally {
      setIsExportingPdf(false)
    }
  }

  const handleExcelExport = async () => {
    try {
      setIsExportingExcel(true)
      const response = await apiClient.get('/inventory/reports/stock-movement-export', {
        params: { report_type: 'excel', batch_master_id: batchId, product_id: productId, movement_type: movementType },
        responseType: 'blob'
      })
      const url = window.URL.createObjectURL(new Blob([response.data]))
      const link = document.createElement('a')
      link.href = url
      link.setAttribute('download', `stock_movement_report_${new Date().toISOString().split('T')[0]}.xlsx`)
      document.body.appendChild(link)
      link.click()
      link.remove()
      window.URL.revokeObjectURL(url)
    } catch (error) {
      console.error('Failed to export Excel:', error)
      showNotificationModal('Error!', 'Failed to export Excel report.', 'error')
    } finally {
      setIsExportingExcel(false)
    }
  }

  const totalPages = Math.ceil((reportData?.recordsFiltered ?? 0) / pageSize)

  const toolbarLeft = (
    <div className="flex items-center gap-3">
        <div className="w-[180px]">
            <Select2
                options={batchOptions}
                value={batchId}
                onChange={(val) => { setBatchId(val as string); setCurrentPage(1) }}
                rounded="full"
                variant="solid"
                placeholder="All Batches"
                isClearable
            />
        </div>
        <div className="w-[200px]">
            <Select2
                options={productOptions}
                value={productId}
                onChange={(val) => { setProductId(val as string); setCurrentPage(1) }}
                rounded="full"
                variant="solid"
                placeholder="All Products"
                isClearable
            />
        </div>
        <div className="w-[200px]">
            <Select2
                options={movementTypeOptions}
                value={movementType}
                onChange={(val) => { setMovementType(val as string); setCurrentPage(1) }}
                rounded="full"
                variant="solid"
                placeholder="Movement Types"
                isClearable
            />
        </div>
    </div>
  )

  const toolbarRight = (
    <div className="flex items-center gap-3">
      <button 
        disabled={isExportingPdf}
        onClick={handlePdfExport}
        className="bg-[#f1f5f9] border border-gray-200 px-4 py-2 rounded-full text-[11px] font-bold text-[#64748b] h-9 flex items-center gap-2 hover:bg-rose-50 hover:text-rose-600 hover:border-rose-200 transition-colors group shrink-0 disabled:opacity-50 disabled:cursor-not-allowed"
      >
        {isExportingPdf ? (
          <Loader2 className="h-4 w-4 animate-spin text-rose-500" strokeWidth={2.5} />
        ) : (
          <FileDown className="h-4 w-4 text-[#64748b] group-hover:text-rose-600 transition-colors" strokeWidth={2.5} />
        )}
        {isExportingPdf ? 'PROCESSING...' : 'PDF'}
      </button>

      <button 
        disabled={isExportingExcel}
        onClick={handleExcelExport}
        className="bg-[#f1f5f9] border border-gray-200 px-4 py-2 rounded-full text-[11px] font-bold text-[#64748b] h-9 flex items-center gap-2 hover:bg-emerald-50 hover:text-emerald-600 hover:border-emerald-200 transition-colors group shrink-0 disabled:opacity-50 disabled:cursor-not-allowed"
      >
        {isExportingExcel ? (
          <Loader2 className="h-4 w-4 animate-spin text-emerald-500" strokeWidth={2.5} />
        ) : (
          <FileSpreadsheet className="h-4 w-4 text-[#64748b] group-hover:text-emerald-600 transition-colors" strokeWidth={2.5} />
        )}
        {isExportingExcel ? 'PROCESSING...' : 'EXCEL'}
      </button>
    </div>
  )

  const tabs = reportCategoryTabs.map(t => ({ ...t, active: t.name === 'Stock' }))

  return (
    <ListPageLayout<StockMovementReportListItem>
      title="Stock Movement Report"
      titleOptions={stockReportOptions}
      tabs={tabs}
      backTo="/"
      showSearch={false}
      showStatusFilter={false}
      showColumnFilter={true}
      columns={filterColumns}
      onColumnToggle={toggleColumn}
      rowData={gridData}
      columnDefs={columnDefs}
      isLoading={isLoading}
      recordsTotal={reportData?.recordsFiltered ?? 0}
      currentPage={currentPage}
      pageSize={pageSize}
      totalPages={totalPages}
      onPageChange={setCurrentPage}
      onPageSizeChange={(size) => { setPageSize(size); setCurrentPage(1) }}
      toolbarExtra={toolbarLeft}
      toolbarRightExtra={toolbarRight}
      gridOptions={{
        postSortRows: (params: any) => {
          const rowNodes = params.nodes;
          const summaryIndex = rowNodes.findIndex((node: any) => node.data?.isSummary);
          if (summaryIndex !== -1) {
            const summaryNode = rowNodes.splice(summaryIndex, 1)[0];
            rowNodes.push(summaryNode);
          }
        },
        getRowStyle: (params: any) => {
          if (params.data?.isSummary) {
            return { 
              backgroundColor: '#f8fafc',
              fontWeight: 'bold',
              borderTop: '2px solid #e2e8f0'
            }
          }
        }
      }}
    />
  )
}
