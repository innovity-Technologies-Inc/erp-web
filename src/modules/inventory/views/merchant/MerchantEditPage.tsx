...
export const MerchantEditPage = () => {
  ...
  const { data: merchantDetails, isLoading: isLoadingDetails } = useMerchantData(id)
  ...
  useEffect(() => {
    if (merchantDetails?.data) {
      ...
      reset({
        ...
        address2: d.address2 || '',
        sales_permit_number: d.sales_permit_number || '',
        status: Number(d.status ?? 1),
        comission_value: d.comission_value || '0.00',
        comission_type: Number(d.comission_type ?? 1),
        comission_note: d.comission_note || '',
        password_option: '',
      })
    }
  ...
  const onSubmit = (data: any) => {
    ...
    if (merchantDetails?.data?.uuid) {
      formData.append('uuid', merchantDetails.data.uuid)
    }
    ...
    if (data.sales_permit?.[0] instanceof File) {
      ...
      <div className="max-w-[1600px] mx-auto pb-6">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-[20px] font-medium text-primary tracking-tight ml-2">
            Edit Merchant <span className="text-gray-400 text-[14px] ml-1">[{merchantDetails?.data?.customer_name}]</span>
          </h1>
        </div>
      </div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-1.5">
                      ...
                      <div className="flex flex-wrap items-center gap-3">
                        ...
                        {merchantDetails?.data?.sales_permit && (
                          (() => {
                            const salesPermit = merchantDetails.data.sales_permit;
                            const previewUrl = salesPermit.includes('href=') 
                              ? salesPermit.match(/href="([^"]+)"/)?.[1] 
                              : `${import.meta.env.VITE_STORAGE_URL}/${salesPermit}`;
                            ...
                            return (
                              <a 
                                ...
                                className="flex items-center gap-2 px-6 h-[42px] bg-primary/10 text-primary hover:bg-primary/20 rounded-lg transition-all font-semibold text-[14px] border border-primary/20"
                              >
                                View File
                        ...
                        <span className="text-[12px] text-gray-400 font-medium w-full md:w-auto">PNG, JPG or WEBP (Max. 2MB)</span>
                      </div>
                    </div>
                  </div>
                </div>
