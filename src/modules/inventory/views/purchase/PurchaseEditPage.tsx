...
import { purchaseSchema, type PurchaseFormValues } from '../../hooks/validation'
import { 
  useUpdatePurchase, 
  usePurchaseData, 
...
const PurchaseItemRow = ({ 
  ...
  // Pass purchaseId to allow backend to adjust stock levels (stock minus this transaction's qty)
  const { data: productData } = useRetrieveProductData(supplierId, itemId, purchaseId || null);

  useEffect(() => {
    if (productData) {
      // ONLY update rate if this is NOT the initial load of the edit form
...
export const PurchaseEditPage = () => {
  ...
  const vendorOptions = useMemo(() => vendors?.map((v: any) => ({ value: Number(v.id), label: v.text })) || [], [vendors])
  const paymentMethodOptions = useMemo(() => paymentMethods?.map((p: any) => ({ value: Number(p.id), label: p.text })) || [], [paymentMethods])

  // Map database data to form values reactively
  const formValues = useMemo(() => {
    const rawData = (purchase as any)?.data || purchase;
    if (!rawData || (!rawData.id && !rawData.uuid)) return undefined;

    const grouped: Record<string, any> = {}
    const details = rawData.product_purchase_details || rawData.productPurchaseDetails || rawData.purchase_details || rawData.details || []

    if (Array.isArray(details)) {
      details.forEach((detail: any) => {
        // Check all possible ID variants
        const productId = detail.product_id || detail.item_id || detail.product?.id || detail.product_purchase_id;
        if (!productId) return;

        const rate = parseFloat(String(detail.rate || detail.purchase_rate || detail.price || 0));
        const expiryDate = detail.expiry_date || '';
        const discount = parseFloat(String(detail.discount || detail.discount_percent || 0));
        
        const key = `${productId}_${rate}_${expiryDate}_${discount}`
        if (!grouped[key]) {
          grouped[key] = {
            ...
            discount_value: parseFloat(String(detail.discount_amnt || detail.discount_amount || detail.discount_value || 0)),
            total: parseFloat(String(detail.total_amount || detail.total || detail.sub_total || 0)),
            warehouses: []
          }
        }

        // Extract warehouse info from multiple possible relationships
        const bp = detail.batch_product || detail.batchProduct || detail.warehouse_stock;
        const warehouseId = bp?.warehouse_id || detail.warehouse_id || detail.batch_product?.warehouse_id;

        if (warehouseId) {
          grouped[key].warehouses.push({
            warehouse_id: Number(warehouseId),
            quantity: parseFloat(String(detail.quantity || detail.qty || detail.stock || 0))
          })
        }
      })
    }
    ...
    // Map header fields with all possible variants
    const supplierIdVal = rawData.supplier_id || rawData.vendor_id || rawData.supplier?.id || rawData.supplier_information_id;
    const purchaseDiscountVal = rawData.invoice_discount || rawData.purchase_discount || rawData.discount || rawData.total_discount || 0;
    const paidAmountVal = rawData.paid_amount || rawData.paid || rawData.payment_amount || 0;
  ...
  // Watch variables for reactivity
  ...
  const currentBatchNo = useMemo(() => {
    const rawData = (purchase as any)?.data || purchase;
    const details = rawData?.product_purchase_details || rawData?.productPurchaseDetails || rawData?.purchase_details || rawData?.details || [];
    return details[0]?.batch_master?.batch_no || details[0]?.batchMaster?.batch_no || details[0]?.batch_no || rawData?.batch_no;
  }, [purchase]);

  // Global Calculations (Reactive)
  const { subTotal, totalItemDiscount, grandTotal, dueAmount } = useMemo(() => {
    ...
    const grand = sub - itemDisc - globalDisc
  ...
  const onSubmit = (data: PurchaseFormValues) => {
    // Only check if it changed from the original chalan_no/batch_no to avoid blocking the current entry
    const rawData = (purchase as any)?.data || purchase;
    if (data.chalan_no !== rawData?.chalan_no && chalanCheck?.chalan_no) {
      showNotificationModal('Chalan Error', 'Chalan No Already Exists!', 'error');
      return;
    }
    const details = rawData?.product_purchase_details || rawData?.productPurchaseDetails || rawData?.purchase_details || rawData?.details || []
    const originalBatchNo = details[0]?.batch_master?.batch_no || details[0]?.batchMaster?.batch_no || details[0]?.batch_no || rawData?.batch_no;
    if (data.batch_no !== originalBatchNo && batchCheck?.batch_no) {
    ...
    <div className="min-h-screen bg-[#f1f0f5] pb-10 font-poppins text-[#475569]">
      <div className="max-w-[1600px] mx-auto pb-6">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-[20px] font-medium text-primary tracking-tight ml-2">Edit Purchase</h1>
        </div>
      </div>
                   ...
                   <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                        <span className="text-[10px] text-gray-400 font-medium leading-tight">
                           ...
                           ) : rawData?.invoice_file ? (
                             <span className="text-primary font-bold truncate max-w-[150px] block">
                               Current: {rawData.invoice_file.split('/').pop()}
                             </span>
                           ) : (
            ...
            <div className="bg-primary rounded-xl p-4 shadow-lg text-white">
               ...
               <div className="pt-4 border-t border-white/20 flex justify-between items-end">
                 ...
                 <div className="px-3 py-1 bg-white/20 rounded-md text-[11px] font-bold">
                    {grandTotal > paidAmount ? 'Partial' : 'Paid'}
                 </div>
               </div>
            </div>
            ...
            <div className="flex gap-4 pt-2">
               ...
               <button type="button" onClick={handleSubmit(onSubmit)} disabled={isPending} className="flex-1 h-[48px] bg-[#0d7a50] hover:bg-[#0a6642] text-white font-bold rounded-xl transition-all shadow-lg shadow-emerald-500/20 flex items-center justify-center gap-2 disabled:opacity-50 text-[15px]">
                 {isPending ? (
                   ...
                   <><Check className="h-5 w-5" strokeWidth={3} /> Update</>
                 )}
               </button>
            </div>
          </div>
