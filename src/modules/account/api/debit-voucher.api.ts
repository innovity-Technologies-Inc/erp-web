...
export interface DebitVoucherListItem {
  id: number
  uuid: string
  v_no: string
  v_date: string
...
export interface DebitVoucherItemPayload {
...
export interface DebitVoucherStorePayload {
  ...
  is_honours?: number
  items: DebitVoucherItemPayload[]
}

export interface DebitVoucherUpdatePayload extends DebitVoucherStorePayload {
  uuid: string
}
...
export interface DebitVoucherDetailsResponse {
  id: number
  uuid: string
  v_no: string
  v_date: string
...
export const debitVoucherApi = {
  getDatatable: async (params: any): Promise<DataTablesResponse<DebitVoucherListItem>> => {
    const response = await apiClient.get<DataTablesResponse<DebitVoucherListItem>>('/account/debit-voucher/datatable', { params })
    return response.data
  },

  store: async (data: DebitVoucherStorePayload): Promise<ApiResponse<any>> => {
    const response = await apiClient.post<ApiResponse<any>>('/account/debit-voucher/store', data)
    return response.data
  },

  update: async (data: DebitVoucherUpdatePayload): Promise<ApiResponse<any>> => {
    const response = await apiClient.post<ApiResponse<any>>('/account/debit-voucher/update', data)
    return response.data
  },

  delete: async (payload: { uuid: string; id: number }): Promise<ApiResponse<any>> => {
    const response = await apiClient.delete<ApiResponse<any>>('/account/debit-voucher/delete', {
      data: payload,
    })
    return response.data
  },

  show: async (id: number | string): Promise<ApiResponse<DebitVoucherDetailsResponse>> => {
    const response = await apiClient.get<ApiResponse<DebitVoucherDetailsResponse>>(`/account/voucher/show/${id}`)
    return response.data
  },
