...
import { 
  useJournalVouchersDatatable, 
  useJournalVoucherDetails, 
  useDeleteJournalVoucher 
} from '../../hooks/useJournalVoucher'
import type { JournalVoucherListItem } from '../../api/journal-voucher.api'

export const JournalVoucherListPage = () => {
  ...
  const { data: rawDataResponse, isLoading } = useJournalVouchersDatatable(params)
  const { mutate: deleteVoucher, isPending: isVoucherDeleting } = useDeleteJournalVoucher()

  // Details Hook
  const { data: voucherDetails, isLoading: isDetailsLoading } = useJournalVoucherDetails(selectedVoucherId)
  ...
  // ── CRUD Event Handlers ──
  const handleCreate = () => {
    navigate({ to: '/account/voucher/journal/create' })
  }

  const handleEdit = (uuid: string) => {
    navigate({ to: `/account/voucher/journal/edit/${uuid}` })
  }
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: `Journal_Voucher_${voucherDetails?.v_no || ''}`,
    onAfterPrint: () => {
      // Cleanups if any
    }
  })
  ...
  // AG Grid Columns
  const columnDefs = useMemo<ColDef<JournalVoucherListItem>[]>(() => [
    {
      headerName: 'SL',
      width: 80,
      valueGetter: (params) => {
      ...
      cellRenderer: (params: any) => {
        ...
        return (
          <div className="flex items-center gap-1.5 h-full">
            ...
            <PermissionGuard permission="edit_journal_voucher">
              {!isApproved && (
                <button
            ...
            <PermissionGuard permission="delete_journal_voucher">
              {!isApproved && (
                <button
  ...
  return (
    <>
      <ListPageLayout<JournalVoucherListItem>
        title="Journal Voucher"
        titleOptions={navOptions}
        tabs={tabs}
        backTo="/account/chart-of-accounts"
        onCreate={handleCreate}
        createPermission="create_journal_voucher"
        addLabel="Add"
        showSearch={true}
        searchValue={search}
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto bg-black/40 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-[900px] flex flex-col max-h-[90vh]">
            ...
            <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100 print:hidden">
              <h3 className="text-lg font-bold text-[#1e293b] font-poppins">Journal Voucher Detail</h3>
              <button 
                onClick={() => setIsDetailsOpen(false)} 
