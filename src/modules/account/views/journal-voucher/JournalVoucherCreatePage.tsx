...
export const JournalVoucherCreatePage = () => {
  const navigate = useNavigate()
  const { showNotificationModal } = useUiStore()

  // Queries & Mutations
  const { data: levelFourAccountHeads = [], isLoading: isAccountsLoading } = useLevelFourAccountHeadsSelect2()
  const { mutate: storeJournalVoucher, isPending: isSaving } = useStoreJournalVoucher()
  ...
  const isDirty = useMemo(() => {
    ...
    if (firstRow.account_id || firstRow.sub_type_id || firstRow.ledger_comment || firstRow.debit || firstRow.credit) return true
    return false
  }, [date, remark, activeRows])
