...
import { 
  useContraVouchersDatatable, 
  useContraVoucherDetails, 
  useDeleteContraVoucher 
} from '../../hooks/useContraVoucher'
import type { ContraVoucherListItem } from '../../api/contra-voucher.api'

export const ContraVoucherListPage = () => {
  ...
  const { data: rawDataResponse, isLoading } = useContraVouchersDatatable(params)
  const { mutate: deleteVoucher, isPending: isVoucherDeleting } = useDeleteContraVoucher()

  // Details Hook
  const { data: voucherDetails, isLoading: isDetailsLoading } = useContraVoucherDetails(selectedVoucherId)
  ...
  // ── CRUD Event Handlers ──
  const handleCreate = () => {
    navigate({ to: '/account/voucher/contra/create' })
  }

  const handleEdit = (uuid: string) => {
    navigate({ to: `/account/voucher/contra/edit/${uuid}` })
  }
      ...
      cellRenderer: (params: any) => {
        ...
        return (
          <div className="flex items-center gap-1.5 h-full">
            ...
            <PermissionGuard permission="edit_contra_voucher">
              {!isApproved && (
                <button
            ...
            <PermissionGuard permission="delete_contra_voucher">
              {!isApproved && (
                <button
  ...
  return (
    <>
      <ListPageLayout<ContraVoucherListItem>
        title="Contra Voucher"
        titleOptions={navOptions}
        tabs={tabs}
        backTo="/account/chart-of-accounts"
        onCreate={handleCreate}
        createPermission="create_contra_voucher"
        addLabel="Add"
        showSearch={true}
        searchValue={search}
      ...
      <ConfirmationModal
        ...
        message="Are you sure you want to delete this contra voucher? This action cannot be undone."
