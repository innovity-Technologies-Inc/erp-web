...
import { 
  ...
  useMerchantsSelect2,
  useServiceVouchersSelect2, 
  useStoreServicePayment 
} from '../../hooks/useServicePayment'
...
export const ServicePaymentPage = () => {
  ...
  const { data: rawVouchers = [], isLoading: isVouchersLoading } = useServiceVouchersSelect2(
  ...
  // Mutation
  const { mutate: storePayment, isPending: isSaving } = useStoreServicePayment()
    ...
    storePayment(payload, {
      onSuccess: (res: any) => {
        if (res.status) {
          ...
          queryClient.invalidateQueries({ queryKey: ['select2', 'service-vouchers'] })
                ...
                <table className="w-full border-collapse text-sm">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {savedVoucherData.items?.map((item: any) => {
                      ...
                      // Fetch/Fallback head name (Service Collection: Dr. Cash/Bank, Cr. Accounts Receivable / Service Revenue)
