...
import { 
  ...
  useVendorsSelect2,
  useVendorVouchersSelect2, 
  useStoreVendorPayment 
...
export const VendorPaymentPage = () => {
  ...
  const { data: rawVendors = [], isLoading: isVendorsLoading, error: vendorsError } = useVendorsSelect2()

  // Safe Console Debugging to diagnose data load issues on the client side
  console.log('[VendorPayment] rawVendors:', rawVendors, 'isLoading:', isVendorsLoading, 'error:', vendorsError)
  ...
  // Fetch Vouchers dynamically when a Vendor is selected
  const { data: rawVouchers = [], isLoading: isVouchersLoading } = useVendorVouchersSelect2(
  ...
  // Mutation
  const { mutate: storePayment, isPending: isSaving } = useStoreVendorPayment()
  ...
  const vendorOptions = useMemo(() => 
    (rawVendors || []).map((v: any) => ({ value: String(v.id), label: v.text })),
    [rawVendors]
  )
  ...
  // Clear voucher selections if vendor changes
  ...
  const validateForm = () => {
    ...
    if (!vendorId) newErrors.vendorId = 'Vendor Name is required'
    ...
    storePayment(payload, {
      onSuccess: (res: any) => {
        if (res.status) {
          ...
          queryClient.invalidateQueries({ queryKey: ['select2', 'vendor-vouchers'] })
          ...
          {!voucherId ? (
            ...
            <div className="border-2 border-dashed border-[#475569]/30 bg-[#113671]/14 rounded-xl p-10 flex flex-col items-center justify-center text-center h-[280px]">
              ...
              <p className="text-[#0f2d4a]/80 text-[14px] italic font-poppins px-2 leading-relaxed">
                "Select vendor name and voucher number."
              </p>
            </div>
          ) : (
            /* Data loaded state */
                ...
                <table className="w-full border-collapse text-sm">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {savedVoucherData.items?.map((item: any) => {
                      ...
                      // Fetch/Fallback head name
                      let particulars = item.acc_coa?.head_name
                      if (!particulars) {
                        if (dbVal > 0) {
                          particulars = `Accounts Payable - ${vendorOptions.find(v => String(v.value) === vendorId)?.label || 'Supplier'}`
                        } else {
