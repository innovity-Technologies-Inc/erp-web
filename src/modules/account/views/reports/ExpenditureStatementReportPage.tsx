...
import { useExpenditureStatementReport } from '@/modules/account/hooks/useReports'
...
export const ExpenditureStatementReportPage = () => {
  ...
  const { data: reportData, isFetching: isLoading } = useExpenditureStatementReport(params)
  ...
  const totals = useMemo(() => {
    ...
    if (expenses[0]) {
      ...
      if (expenses[0].nextlevel) {
        expenses[0].nextlevel.forEach((group: any) => {
          const isCogs = 
            String(group.headCode || '').startsWith('401') || 
            group.headName?.toLowerCase().includes('cost of goods') ||
            group.headName?.toLowerCase().includes('cogs')

          if (isCogs) {
            totalCogs += parseFloat(String(group.subtotal || 0))
          } else {
            totalOverhead += parseFloat(String(group.subtotal || 0))
          }
        })
      }
    }
  ...
  const handlePdfExport = async () => {
    try {
      ...
      const response = await apiClient.post('/account/report/expenditure-statement-export', {
        report_type: 'pdf',
        fromDate,
        toDate,
      }, { responseType: 'blob' })
  ...
  const handleExcelExport = async () => {
    try {
      ...
      link.href = url
      link.setAttribute('download', `expenditure_statement_report_${new Date().toISOString().split('T')[0]}.xlsx`)
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          >
            ...
            <div className="flex items-baseline justify-between">
              ...
              <span 
                style={{
                  backgroundColor: hoveredCard === 1 ? 'rgba(255,255,255,0.2)' : '#fdf2f2',
                  color: hoveredCard === 1 ? '#ffffff' : '#dc2626',
                }}
          ...
          >
            ...
            <div className="flex items-baseline justify-between">
              ...
              <span 
                style={{
                  backgroundColor: hoveredCard === 2 ? 'rgba(255,255,255,0.2)' : '#f2f4f6',
                  color: hoveredCard === 2 ? '#ffffff' : '#003671',
                }}
                className="text-[10px] font-bold px-2.5 py-1 rounded-full transition-all"
              >
                COGS
          ...
          >
            ...
            <div className="flex items-baseline justify-between">
              <div 
                ...
                className="text-[18px] font-extrabold tracking-tight transition-colors"
              >
                {formatCurrency(Math.round(totals.totalOverhead), currency, currencyPosition).split('.')[0]}
              </div>
              <span 
                style={{
                  ...
                  expenses.map((expense: any, idx: number) => (
                    <Fragment key={idx}>
                      ...
                      {expense.nextlevel?.map((value: any, valIdx: number) => (
                        <Fragment key={valIdx}>
                          {/* Level 2 Subgroup (e.g. Administrative Expenses) */}
