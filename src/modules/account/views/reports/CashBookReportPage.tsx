...
export const CashBookReportPage = () => {
  ...
  const { data: cashHeads = [], isLoading: isHeadsLoading } = useCashHeads()
  ...
  const handlePdfExport = async () => {
    if (!bankType) {
      showNotificationModal('Warning!', 'Please select a Cash Head first.', 'warning')
      return
    }
    try {
  ...
  const handleExcelExport = async () => {
    ...
    try {
      ...
      link.href = url
      link.setAttribute('download', `cash_book_report_${new Date().toISOString().split('T')[0]}.xlsx`)
