...
import { useCashBookReportDatatable, useBankHeads } from '@/modules/account/hooks/useReports'
...
export const BankBookReportPage = () => {
  ...
  const { data: bankHeads = [], isLoading: isHeadsLoading } = useBankHeads()
  ...
  const handlePdfExport = async () => {
    if (!bankType) {
      showNotificationModal('Warning!', 'Please select a Bank Head first.', 'warning')
      return
    }
    try {
  ...
  const handleExcelExport = async () => {
    ...
    try {
      ...
      link.href = url
      link.setAttribute('download', `bank_book_report_${new Date().toISOString().split('T')[0]}.xlsx`)
