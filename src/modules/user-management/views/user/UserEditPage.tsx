...
export const UserEditPage = () => {
  ...
  const { uuid } = useParams({ from: '/_authenticated/user/edit/$uuid' })
  ...
  const { mutate: updateUserMutation, isPending: isSaving } = useUpdateUser(uuid)
  ...
  const onSubmit = (data: UserEditFormValues) => {
    ...
    updateUserMutation(formData, {
      onSuccess: () => {
        showNotificationModal('Saved Successfully!', 'The user details have been updated successfully.', 'success')
        navigate({ to: '/user' })
      },
      onError: (error: any) => {
    ...
    return (
      <div className="min-h-screen bg-[#f1f0f5] flex items-center justify-center font-poppins">
        <div className="flex flex-col items-center gap-3">
          ...
          <span className="text-sm font-semibold text-gray-500">Loading user details...</span>
        </div>
      </div>
    )
  }
    ...
    <div className="min-h-screen bg-[#f1f0f5] pb-10 font-poppins text-[#475569]">
      ...
      <div className="max-w-[1600px] mx-auto pb-6">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-[20px] font-medium text-primary tracking-tight ml-2">Edit User</h1>
        </div>
      </div>
