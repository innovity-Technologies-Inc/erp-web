...
import { ArrowLeft, Check, Briefcase, Info } from 'lucide-react'
import { useState, useMemo, useEffect } from 'react'
import { roleSchema } from '../../hooks/validation'
import type { RoleFormValues } from '../../hooks/validation'
import { useUpdateRole, useRoleDetails, usePermissionsList } from '../../hooks/useRoles'
...
export const RoleEditPage = () => {
  ...
  const { uuid } = useParams({ from: '/_authenticated/role/edit/$uuid' })
  ...
  const { data: roleDetails, isLoading: isLoadingDetails } = useRoleDetails(uuid)
  const { mutate: updateRole, isPending: isSaving } = useUpdateRole(uuid)
  ...
  // Pre-fill form when roleDetails is fetched
  useEffect(() => {
    if (roleDetails?.role) {
      setValue('name', roleDetails.role.name)
      if (roleDetails.role.organization_id) {
        setValue('organization_id', String(roleDetails.role.organization_id))
      }
      if (roleDetails.role.company_id) {
        setValue('company_id', String(roleDetails.role.company_id))
      }
      if (roleDetails.role.permissions) {
        const ids = roleDetails.role.permissions.map((p: any) => p.id)
        setSelectedIds(ids)
      }
    }
  ...
  if (isLoadingPermissions || isLoadingDetails) {
    return <LoadingState />
  }

  return (
    <div className="min-h-screen bg-[#f1f0f5] font-poppins text-[#475569]">
      {/* Header */}
      <div className="max-w-[1600px] mx-auto pb-6">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-[20px] font-medium text-primary tracking-tight ml-2">Update Role</h1>
        </div>
      </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-6">
            <div className="flex items-start justify-between pb-4 border-b border-gray-50">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-[16px] font-semibold text-slate-800 tracking-tight">Update Role</h2>
                  <p className="text-[12px] text-gray-400 font-medium">Modify the role name and adjust access permissions.</p>
                </div>
              </div>
